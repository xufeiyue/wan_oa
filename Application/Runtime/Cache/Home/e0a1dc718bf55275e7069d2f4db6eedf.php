    ...
    <ul class="forminfo">
        <li><label>所属部门</label>
            <select name="department_id" id="department_id" class="dfinput">
                ...
                <?php if(is_array($departmentList)): $i = 0; $__LIST__ = $departmentList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php if($vo['id'] == $info['department_id']): ?>selected='selected'<?php endif; ?>><?php echo ($vo['department']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </li>
        <li><label>员工姓名</label><input name="truename" type="text" class="dfinput" id="truename" value="<?php echo ($info['truename']); ?>"/></li>
